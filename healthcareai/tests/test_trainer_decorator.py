...
class TestTrainerDecorator(unittest.TestCase):
    ...
    We will compare a decorated linear regression and a undecorated linear
    regression output. They should not be the same.
    """
    ...
    def setUp(self):
        ...
        # Fill in missing data so trainer doesn't error out
        df['SystolicBPNBR'] = df['SystolicBPNBR'].fillna(149)

        self.regression_trainer = SupervisedModelTrainer(
            df,
            'SystolicBPNBR',
            'regression',
        ...
        self.regression_trainer.undecorated_lr = undecorated_lr.__get__(
